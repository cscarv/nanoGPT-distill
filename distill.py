...
# teacher models
eng_teacher_path = None # path to the English teacher model checkpoint
fr_teacher_path = None # path to the French teacher model checkpoint
# adamw optimizer
...
# Load teacher models
eng_ckpt_path = os.path.join(eng_teacher_path, 'ckpt.pt')
fr_ckpt_path = os.path.join(fr_teacher_path, 'ckpt.pt')
eng_ckpt = torch.load(eng_ckpt_path, map_location=device)
fr_ckpt = torch.load(fr_ckpt_path, map_location=device)
eng_teacher_args = eng_ckpt['model_args']
fr_teacher_args = fr_ckpt['model_args']
# Create teacher models with the same config as the original models
eng_teacher_conf = GPTConfig(**model_args)
fr_teacher_conf = GPTConfig(**model_args)
eng_teacher = GPT(eng_teacher_conf).to(device)
fr_teacher = GPT(fr_teacher_conf).to(device)
# Load the state dicts into the teacher models
eng_state_dict = eng_ckpt['model']
fr_state_dict = fr_ckpt['model']
# Fix the keys of the state dictionaries if needed
unwanted_prefix = '_orig_mod.'
for k,v in list(eng_state_dict.items()):
    if k.startswith(unwanted_prefix):
        eng_state_dict[k[len(unwanted_prefix):]] = eng_state_dict.pop(k)
for k,v in list(fr_state_dict.items()):
    if k.startswith(unwanted_prefix):
        fr_state_dict[k[len(unwanted_prefix):]] = fr_state_dict.pop(k)
eng_teacher.load_state_dict(eng_state_dict)
fr_teacher.load_state_dict(fr_state_dict)
# switch to eval mode for teachers
eng_teacher.eval()
fr_teacher.eval()
# disable gradients for teachers
for param in eng_teacher.parameters():
    param.requires_grad = False
for param in fr_teacher.parameters():
    param.requires_grad = False
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        with ctx:
            ...
            # compute the logits and log-probs from the English teacher
            with torch.no_grad():
                eng_teacher_logits, _ = eng_teacher(X, Y)
